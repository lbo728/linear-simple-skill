...
export class AnthropicProvider implements LLMProvider {
  ...
  /**
   * Generate a structured blog draft from branch analysis using Anthropic Claude
   * 
  ...
  async generateDraft(input: LLMInput): Promise<BlogDraft | null> {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        console.log(`[LLM] Token usage - Input: ${usage.input_tokens}, Output: ${usage.output_tokens}, Total: ${usage.input_tokens + usage.output_tokens}`);
        ...
        const draft = this.parseResponse(content.text);
  ...
  /**
   * Build the prompt for Anthropic based on branch analysis
   */
